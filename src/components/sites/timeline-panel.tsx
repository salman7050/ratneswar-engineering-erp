...
import { addTimelineEvent, deleteTimelineEvent } from "@/lib/actions/site-ops-actions";
...
export function TimelinePanel({ siteId, events }: { siteId: string; events: SiteDetail["timeline"] }) {
  const [open, setOpen] = React.useState(false);
  const [form, setForm] = React.useState({ title: "", description: "", category: "GENERAL", eventDate: new Date().toISOString().slice(0, 10) });

  const { run: add, loading } = useAction(addTimelineEvent, {
  ...
  const { run: remove } = useAction(deleteTimelineEvent, { successMessage: "Event removed" });
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-1.5"><Label>Title</Label><Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Grid synchronization completed" /></div>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                loading={loading}
                disabled={!form.title}
                onClick={() => add({ siteId, title: form.title, description: form.description || null, category: form.category as any, eventDate: new Date(form.eventDate) })}
              >
                Add
              </Button>
            </DialogFooter>
            ...
            return (
              <li key={e.id} className="relative">
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button onClick={() => remove(e.id, siteId)} className="shrink-0 text-muted-foreground hover:text-destructive">
