...
export function AdminAiCard() {
  ...
  return <div className="relative overflow-hidden rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-600 via-indigo-600 to-blue-700 p-5 text-white shadow-[0_16px_40px_rgba(79,70,229,.22)]">
    ...
    <div className="relative"><div className="flex items-start justify-between"><div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-violet-100"><Sparkles className="h-4 w-4" /> Admin Only</div><h2 className="mt-2 text-xl font-bold">AI Operations Assistant</h2><p className="mt-1 max-w-md text-sm text-violet-100">Draft documents, search records and analyse business data. Cloud AI runs without any office-PC server and uses the configured free cloud quota.</p></div><WandSparkles className="h-8 w-8 text-violet-100" /></div>
      ...
      <Button onClick={() => open()} variant="secondary" className="mt-4 w-full bg-white text-indigo-700 hover:bg-violet-50">Open AI Workspace</Button>
    </div>
  </div>;
}
