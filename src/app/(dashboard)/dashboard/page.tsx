...
function Metric({ icon: Icon, value, label, href, tone }: { icon: typeof Building2; value: number; label: string; href: string; tone: "blue" | "green" | "amber" | "violet" | "cyan" }) {
  const styles = {
  ...
  return (
    <Link href={href} className="group flex min-h-[96px] items-center gap-4 rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md">
      <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${styles[tone]}`}><Icon className="h-5 w-5" /></span>
...
export default async function DashboardPage() {
  ...
  return (
    <div className="min-h-full bg-[#f6f8fb] px-4 py-5 md:px-6 lg:px-7">
      <div className="mx-auto max-w-[1600px] space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          ...
          <Metric icon={ClipboardCheck} value={data.counts.pendingWorks} label="Pending Works" href="/dashboard" tone="amber" />
          <Metric icon={Building2} value={data.counts.activeSites} label="Active Sites" href="/sites" tone="violet" />
          <Metric icon={Receipt} value={data.counts.recentInvoices} label="Recent Invoices" href="/invoices" tone="green" />
          <Metric icon={ShoppingCart} value={data.counts.openPurchaseOrders} label="Open Purchase Orders" href="/purchase-orders" tone="cyan" />
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[1fr_1fr_.78fr]">
          ...
          <Card className="border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 space-y-2">
              ...
              <Button asChild variant="outline" className="h-11 w-full justify-start"><Link href="/quotations"><FileText className="h-4 w-4 text-violet-600" /> New Quotation <ArrowRight className="ml-auto h-3.5 w-3.5 text-slate-300" /></Link></Button>
              <Button asChild variant="outline" className="h-11 w-full justify-start"><Link href="/expenses"><WalletCards className="h-4 w-4 text-amber-600" /> Record Expense <ArrowRight className="ml-auto h-3.5 w-3.5 text-slate-300" /></Link></Button>
