...
export function AssistantPanel({ open, onOpenChange, initialPrompt = "" }: { open: boolean; onOpenChange: (v: boolean) => void; initialPrompt?: string }) {
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent side="right" className="flex w-full flex-col gap-0 border-l border-slate-200 bg-white p-0 sm:max-w-lg">
    <div className="border-b bg-slate-950 px-5 py-5 text-white"><div className="flex items-center gap-3"><div className="rounded-xl bg-white/10 p-2.5"><Sparkles className="h-5 w-5 text-violet-300" /></div><div><p className="font-semibold">Ratneswar AI</p><p className="text-xs text-slate-300">Owner + Admin · {provider}</p></div></div></div>
    ...
    <div ref={scrollRef} className="flex-1 overflow-y-auto"><div className="space-y-4 p-4">{messages.length === 0 && <div><p className="mb-3 text-sm leading-6 text-slate-600">Hindi, Hinglish ya English me bolo. Financial details AI kabhi guess nahi karega.</p><div className="grid gap-2">{QUICK_ACTIONS.map(({ icon: Icon, label, prompt }) => <button key={label} onClick={() => setInput(prompt)} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-3 text-left text-sm font-medium text-slate-700 transition hover:border-violet-200 hover:bg-violet-50"><span className="rounded-lg bg-violet-50 p-2 text-violet-700"><Icon className="h-4 w-4" /></span>{label}</button>)}</div></div>}
      ...
      {loading && <div className="rounded-xl border border-violet-100 bg-violet-50 px-3 py-2.5 text-sm text-violet-800"><div className="flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />Ratneswar AI is working in the cloud…</div></div>}
    </div></div>
    <form onSubmit={(e) => { e.preventDefault(); void send(input); }} className="border-t bg-white p-3"><div className="flex items-center gap-2"><Input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Hindi / Hinglish / English me bolo…" disabled={loading} className="h-11" /><Button type="submit" size="icon" className="h-11 w-11 bg-violet-700 hover:bg-violet-800" disabled={loading || !input.trim()}><Send className="h-4 w-4" /></Button></div><p className="mt-2 text-center text-[10px] text-slate-400">Cloud AI · free daily quota · no office PC required</p></form>
  </SheetContent></Sheet>;
}
