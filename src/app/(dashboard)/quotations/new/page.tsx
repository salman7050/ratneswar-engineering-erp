...
export default async function NewQuotationPage({ searchParams }: { searchParams?: { tenderId?: string } }) {
  ...
  return <SmartQuotationEditor clients={clients} sites={sites} bankAccounts={bankAccounts} canUseAI={user.role === "ADMIN" || user.role === "OWNER"} defaultTerms={company.defaultQuoteTerms} defaultValidityDays={company.quotationValidityDays} signatures={signatures} presetTenderId={searchParams?.tenderId} />;
}
