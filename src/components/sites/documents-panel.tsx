...
import { addSiteDocument, deleteSiteDocument } from "@/lib/actions/site-docs-expenses-actions";
...
const CATEGORIES = [
  "WORK_ORDER", "AGREEMENT", "INVOICE_BILL", "QUOTATION", "TESTING_REPORT",
  "STAFF_DOCUMENT", "ATTENDANCE", "TENDER_DOCUMENT", "INSURANCE", "OTHER",
] as const;

export function DocumentsPanel({ siteId, documents }: { siteId: string; documents: SiteDetail["documents"] }) {
  const router = useRouter();
  const [category, setCategory] = React.useState<(typeof CATEGORIES)[number]>("WORK_ORDER");
  const { run: remove } = useAction(deleteSiteDocument, { successMessage: "Document removed" });

  async function handleUploaded(files: UploadedFile[]) {
    for (const file of files) {
      await addSiteDocument({
        siteId,
        name: file.name,
        category,
        fileUrl: file.path,
        ...
        <ul className="flex flex-col gap-2">
          {documents.map((document) => (
            <li key={document.id} className="flex items-center justify-between gap-3 rounded-lg border border-border bg-secondary/30 px-3 py-2.5">
              ...
              <button onClick={() => remove(document.id, siteId)} className="shrink-0 text-muted-foreground hover:text-destructive" aria-label={`Delete ${document.name}`}>
